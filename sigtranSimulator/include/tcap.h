...
#define TAG_MSISDN				0x82    // Tag: MSISDN tag
#define TAG_MAP_RETURN_RES		0xa2    // Tag: Return result last
#define TAG_SEND_ROUT_INFO      0x2d    // Tag: Send routing info for SM
...
typedef struct _sms_fields_t
{
    ...
    unsigned char SMSC_timestamp[MAX_TIMESTAMP_LEN];
    char sms_data[160];
 ...
 */
unsigned char *forward_sm_MT_packet (sms_fields * input_data, int *message_length, char *imsi);
 ...
 * from a GSM SMS.
 */
char *parse_tcap_sms (char *my_databuffer, MYSQL *msqlHandle, int *trx_id);
